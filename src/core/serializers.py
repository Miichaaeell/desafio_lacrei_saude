...
class ProfessionalSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        email = (attrs.get("email") if "email" in attrs else (self.instance.email if self.instance else "")) or ""
...
class ConsultationSerializer(serializers.ModelSerializer):
    ...
    def validate_scheduled_at(self, value):
        if value < timezone.now() - timedelta(minutes=1):
            raise serializers.ValidationError("A data e hora de agendamento não pode ser menor que a data e hora atual.")
        return value